"""
Migration Script: Transfer Non-Admin Agents to Admin User

This script transfers ownership of all agents belonging to non-admin users
to the admin user (loic.ricci@gmail.com).

Usage:
    python migrate_agents_to_admin.py

WARNING: This will change the owner_user_id of all non-admin user agents!
"""
...
def main():
    print("=" * 60)
    print("AGENT OWNERSHIP MIGRATION TO ADMIN")
    print("=" * 60)
    print()
    ...
    try:
        ...
        # Confirm
        response = input(f"Rename and transfer {len(agents)} agent(s) to admin? (yes/no): ")
        if response.lower() != 'yes':
            print("Migration cancelled.")
            return
        
        print()
        print("Renaming and transferring agents...")
        print("-" * 60)
        ...
        print()
        print("NEXT STEPS:")
        print("1. Non-admin users can now edit their profiles")
        print("2. New agents will be created with their original handles")
        print("3. Old agent data is preserved in 'archived-' agents")
        print("4. Admin owns all archived agents and their training data")
