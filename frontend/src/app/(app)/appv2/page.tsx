...
// Brand Colors - Light Theme (from creators page)
...
function AveeFeedCard({ item, onMarkRead }: { item: FeedItem; onMarkRead: (agentId: string) => void }) {
  ...
  return (
    <div className="group relative overflow-hidden rounded-2xl border border-gray-200 bg-white p-6 shadow-sm transition-all hover:shadow-lg hover:border-[#1F6BFF]/20">
      {/* Update count badge */}
      <div className="absolute right-6 top-6 flex flex-col items-center gap-1">
        <div className="flex h-10 w-10 items-center justify-center rounded-full bg-[#4FB3FF] text-lg font-bold text-white shadow-md">
      ...
      {/* Own agent badge */}
      {item.is_own_agent && (
        <div className="absolute left-6 top-6 rounded-lg bg-[#1F6BFF]/10 px-3 py-1 text-xs font-semibold text-[#1F6BFF]">
          Your Agent
        </div>
      )}
      ...
      <div className="mb-4 flex items-start gap-4" style={{ marginTop: item.is_own_agent ? '2rem' : '0' }}>
        ...
        <div className="h-16 w-16 shrink-0 overflow-hidden rounded-xl border-2 border-gray-200 bg-gradient-to-br from-[#f8fafc] to-gray-200 flex items-center justify-center shadow-sm">
          {item.agent_avatar_url ? (
            ...
            <svg className="h-8 w-8 text-[#1F6BFF]/40" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      {/* Latest update box */}
      {item.latest_update ? (
        <div className="mb-4 rounded-lg border border-[#1F6BFF]/20 bg-gradient-to-r from-[#1F6BFF]/5 to-[#f8fafc] px-4 py-3">
          <div className="flex items-start justify-between gap-2 mb-1">
            <p className="text-sm font-bold text-[#1F6BFF]">{item.latest_update.title}</p>
            {!item.latest_update.is_read && (
              <span className="shrink-0 inline-block h-2 w-2 rounded-full bg-[#4FB3FF]" title="Unread" />
            )}
          </div>
          <p className="text-sm text-gray-900 line-clamp-2">{item.latest_update.content}</p>
          {item.latest_update.topic && (
            <span className="mt-2 inline-block rounded-full bg-[#1F6BFF]/10 px-2 py-1 text-xs text-[#1F6BFF]">
              {item.latest_update.topic}
            </span>
          )}
        </div>
      ...
      <div className="flex gap-2">
        <ChatButton
          ...
          className="flex-1 flex items-center justify-center gap-2 rounded-lg bg-[#1F6BFF] px-6 py-3 text-sm font-semibold text-white shadow-md shadow-[#1F6BFF]/25 transition-all hover:bg-[#0B1220] hover:shadow-lg hover:scale-[1.02]"
        >
        ...
        {item.unread_count > 0 && (
          <button
            ...
            className="shrink-0 rounded-lg border border-[#1F6BFF] px-4 py-3 text-sm font-medium text-[#1F6BFF] transition-colors hover:bg-[#1F6BFF] hover:text-white"
...
function FeedPostCard({ item, onLike, onComment, onRepost, currentUserId, currentUserHandle, currentUserAvatar }: { 
  ...
  return (
    <>
    <div className="group relative overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm transition-all hover:shadow-lg hover:border-[#1F6BFF]/20">
      {/* Repost Header - Show if this is a repost */}
      {isRepost && (
        <div className="px-4 pt-4 pb-2 flex items-center gap-2 text-sm text-gray-600 border-b border-gray-200/50 bg-[#f8fafc]">
      ...
      {isRepost && (item as any).repost_comment && (
        <div className="px-4 py-3 text-sm text-gray-900 italic border-l-4 border-[#1F6BFF]/30 ml-4 mr-4 mt-3 pl-3 bg-[#1F6BFF]/5 rounded-r-lg">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="h-12 w-12 shrink-0 overflow-hidden rounded-full border-2 border-gray-200 bg-gradient-to-br from-[#f8fafc] to-gray-200 flex items-center justify-center shadow-sm">
            {item.agent_avatar_url ? (
              ...
              <svg className="h-6 w-6 text-[#1F6BFF]/40" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
              </svg>
            )}
          </div>
          ...
          {/* Post type badge */}
          <div className="shrink-0 rounded-lg bg-[#1F6BFF]/10 px-2 py-1 text-xs font-semibold text-[#1F6BFF]">
      ...
      <div className="p-4 flex items-center justify-between border-b border-gray-200">
        <div className="flex items-center gap-4">
          ...
          <button
            onClick={handleLike}
            disabled={isLiking}
            className={`flex items-center gap-1.5 text-sm transition-colors hover:text-[#4FB3FF] ${isLiking ? "opacity-50 cursor-not-allowed" : ""}`}
            style={{ color: optimisticLiked ? '#4FB3FF' : '#64748b' }}
          >
            <svg 
          ...
          <button
            ...
            className="flex items-center gap-1.5 text-sm text-gray-500 transition-colors hover:text-[#1F6BFF]"
          >
        ...
        <div className="bg-white rounded-2xl shadow-xl max-w-lg w-full mx-4 p-6" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="mb-4 p-3 border border-gray-200 rounded-lg bg-gray-50">
            <div className="flex items-center gap-2 mb-2">
              <div className="h-8 w-8 rounded-full overflow-hidden bg-[#1F6BFF] flex items-center justify-center">
                {item.agent_avatar_url ? (
          ...
          <div className="mb-4">
            ...
            <textarea
              ...
              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#1F6BFF] focus:border-transparent resize-none"
              rows={3}
            />
          </div>
          ...
          <div className="flex gap-3">
            ...
            <button
              ...
              className="flex-1 px-4 py-2 bg-[#1F6BFF] text-white rounded-lg font-medium hover:bg-[#0B1220] transition-colors"
            >
              Repost
            </button>
          </div>
    ...
    <div className="group relative overflow-hidden rounded-2xl border border-gray-200 bg-white p-6 shadow-sm transition-all hover:shadow-lg hover:border-[#1F6BFF]/20">
      ...
      <div className="mb-4 flex items-start gap-4">
        ...
        {!item.is_read && (
          <div className="shrink-0 flex flex-col items-center">
            <span className="inline-block h-3 w-3 rounded-full bg-[#4FB3FF]" title="Unread" />
          </div>
        )}
      </div>
      ...
      <div className="mb-4 rounded-lg border border-[#1F6BFF]/20 bg-gradient-to-r from-[#1F6BFF]/5 to-[#f8fafc] px-4 py-3">
        <div className="flex items-start justify-between gap-2 mb-2">
          <p className="text-base font-bold text-[#1F6BFF]">{item.title}</p>
        </div>
        <p className="text-sm text-gray-900 line-clamp-4">{item.content}</p>
        {item.topic && (
          <span className="mt-3 inline-block rounded-full bg-[#1F6BFF]/10 px-3 py-1 text-xs font-medium text-[#1F6BFF]">
            {item.topic}
          </span>
        )}
      </div>
  ...
  return (
    <div className="w-80 shrink-0 space-y-6">
      ...
      <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
        <div className="bg-gradient-to-br from-[#1F6BFF] to-[#4FB3FF] p-4">
        ...
        <div className="p-4">
          {profile ? (
            <div className="space-y-3">
              <div className="flex items-center gap-3">
                <div className="h-12 w-12 shrink-0 rounded-full border-2 border-[#1F6BFF]/20 bg-gradient-to-br from-[#1F6BFF] to-[#4FB3FF] flex items-center justify-center text-white font-semibold overflow-hidden">
                  {hasValidAvatar ? (
                    <img 
              ...
              <div className="space-y-2">
                <Link
                  ...
                  className="flex items-center justify-center gap-2 rounded-lg border border-gray-200 px-4 py-2 text-sm font-medium text-gray-900 transition-colors hover:border-[#1F6BFF] hover:bg-[#1F6BFF]/5"
                >
                  View profile
                  <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <ChatButton
                  ...
                  className="w-full flex items-center justify-center gap-2 rounded-lg bg-[#1F6BFF] px-4 py-2 text-sm font-semibold text-white shadow-lg shadow-[#1F6BFF]/25 transition-colors hover:bg-[#0B1220]"
                >
                  <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
                  </svg>
      ...
      <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
        <div className="border-b border-gray-200 bg-[#f8fafc] p-4">
          ...
          <p className="mt-1 text-xs text-gray-500">Discover new Agents</p>
        </div>
            ...
            filteredRecommendations.map((rec) => {
              ...
              return (
                <div key={rec.id} className="pt-4 first:pt-0">
                  <div className="flex items-start gap-3 mb-2">
                    {/* Avatar */}
                    <div className="h-10 w-10 shrink-0 overflow-hidden rounded-lg border-2 border-gray-200 bg-gradient-to-br from-[#1F6BFF] to-[#4FB3FF] flex items-center justify-center">
                      {hasValidRecAvatar ? (
                        <img 
                          src={rec.avatar_url!} 
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs text-gray-500 truncate">@{rec.handle}</div>
                    </div>
                  </div>
                  {rec.bio && (
                  ...
                  <button 
                    ...
                    className="w-full rounded-lg border border-[#1F6BFF] px-3 py-2 text-xs font-medium text-[#1F6BFF] transition-colors hover:bg-[#1F6BFF] hover:text-white"
                  >
                    Follow
                  </button>
                </div>
...
export default function AppV2HomePage() {
        ...
        <main className="flex-1">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-gray-500">
                ...
                {feedData && feedData.total_unread > 0 && (
                  <span className="ml-2 inline-flex items-center gap-1 rounded-full bg-[#4FB3FF]/10 px-2 py-0.5 text-xs font-semibold text-[#4FB3FF]">
                    <span className="inline-block h-1.5 w-1.5 rounded-full bg-[#4FB3FF]"></span>
          ...
          {loading || isLoadingFeed ? (
            ...
            <div className="rounded-2xl border border-gray-200 bg-white p-12 text-center">
              <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-[#1F6BFF]/10">
                <svg className="h-8 w-8 text-[#1F6BFF]/40" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <div className="flex justify-center gap-3">
                <Link
                  href="/network"
                  className="rounded-lg border border-[#1F6BFF] px-4 py-2 text-sm font-medium text-[#1F6BFF] transition-colors hover:bg-[#1F6BFF] hover:text-white"
                >
                  Discover Agents
                </Link>
                <Link
                  href="/my-agents"
                  className="rounded-lg bg-[#1F6BFF] px-4 py-2 text-sm font-medium text-white shadow-lg shadow-[#1F6BFF]/25 transition-colors hover:bg-[#0B1220]"
                >
                  Create Agent
                </Link>
              </div>
