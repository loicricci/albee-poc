...
import { getMyProfile, saveMyProfile, deleteMyAccount, getTwitterConfig, initiateTwitterOAuth, disconnectTwitter, getLinkedInConfig, initiateLinkedInOAuth, disconnectLinkedIn } from "@/lib/api";
...
function ProfileContent() {
  ...
  const [linkedinConfig, setLinkedinConfig] = useState<{ connected: boolean; linkedin_username?: string; organizations?: Array<{ id: string; name: string }> } | null>(null);
  const [connectingLinkedin, setConnectingLinkedin] = useState(false);
  const [includeLinkedinOrgScope, setIncludeLinkedinOrgScope] = useState(false);
  ...
  useEffect(() => {
    ...
    const loadLinkedInConfig = async () => {
      try {
        ...
        // Not connected or error - that's ok
        if (alive) {
          setLinkedinConfig({ connected: false });
        }
      }
    };
  ...
  // Check for LinkedIn OAuth callback status in URL
  useEffect(() => {
    const linkedinStatus = searchParams.get('linkedin');
    const username = searchParams.get('username');
    const reason = searchParams.get('reason');
    
    if (linkedinStatus === 'connected' && username) {
      setOk(`Successfully connected to LinkedIn as ${username}!`);
      // Reload LinkedIn config
      getLinkedInConfig().then(config => {
        if (config) {
          setLinkedinConfig(config);
        }
      }).catch(console.error);
      ...
      setError(`Failed to connect LinkedIn: ${reason || 'Unknown error'}`);
      // Clean up URL
      router.replace('/profile');
    }
  }, [searchParams, router]);
  ...
  async function handleConnectLinkedIn() {
    ...
    try {
      const result = await initiateLinkedInOAuth(includeLinkedinOrgScope);
      // Store the state in session storage for verification (optional)
      if (result.state) {
      ...
      setError("Failed to connect LinkedIn: " + (err.message || "Unknown error"));
      setConnectingLinkedin(false);
    }
  }

  async function handleDisconnectLinkedIn() {
    if (!confirm("Disconnect LinkedIn? Your agents will no longer be able to post to LinkedIn.")) {
      return;
    }
    ...
    try {
      ...
      setLinkedinConfig({ connected: false });
      setOk("LinkedIn disconnected successfully");
    } catch (err: any) {
      setError("Failed to disconnect LinkedIn: " + (err.message || "Unknown error"));
    }
  }
        ...
        <div className="space-y-6 p-6">
          ...
          <div>
            <div className="mb-4">
              ...
              <p className="text-xs text-[#001f98]/70 mt-1">Connect your LinkedIn account to enable agent posting</p>
            </div>
            ...
            {linkedinConfig?.connected ? (
              <div className="rounded-lg border border-green-200 bg-green-50 p-4">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="font-medium text-green-900">
                        Connected as {linkedinConfig.linkedin_username}
                      </div>
                      <div className="text-sm text-green-700">
                        Your agents can now post to LinkedIn
                        {linkedinConfig.organizations && linkedinConfig.organizations.length > 0 && (
                          <span className="ml-1">({linkedinConfig.organizations.length} company page{linkedinConfig.organizations.length > 1 ? 's' : ''} available)</span>
                        )}
                      </div>
                    </div>
                  </div>
              ...
              <div className="rounded-lg border border-blue-200 bg-blue-50 p-4">
                <div className="flex items-start gap-3 mb-3">
                  ...
                  <div className="text-sm text-blue-800">
                    <strong>Connect your LinkedIn account</strong> to enable your agents to post automatically or with your approval.
                    Each agent can be configured independently.
                  </div>
                </div>
                
                {/* Option to request organization posting scope */}
                <div className="mb-4">
                  <label className="flex items-center gap-2 text-sm text-gray-700">
                    <input
                      type="checkbox"
                      checked={includeLinkedinOrgScope}
                      onChange={(e) => setIncludeLinkedinOrgScope(e.target.checked)}
                ...
                <button
                  ...
                  className="flex items-center gap-2 rounded-lg bg-[#0A66C2] px-4 py-2.5 text-sm font-medium text-white transition-colors hover:bg-[#004182] disabled:opacity-50"
                >
                  <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/>
                  </svg>
                  {connectingLinkedin ? "Connecting..." : "Connect LinkedIn Account"}
                </button>
              </div>
            )}
          </div>
